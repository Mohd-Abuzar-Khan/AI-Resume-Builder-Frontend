                ...
                <div class="text-left">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                      @if (selectedJob()?.verdict) {
                        <div class="p-5 rounded-2xl bg-indigo-500/5 border border-indigo-500/20 mb-2">
                          <p class="text-[11px] font-bold uppercase text-indigo-400 mb-2 tracking-widest">ATS Verdict</p>
                          <p class="text-[13px] text-indigo-100/80 font-medium leading-relaxed">{{ selectedJob()?.verdict }}</p>
                        </div>
                      }
                      @if (selectedJob()?.strengths) {
                        <div class="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
                          ...
                          <p class="text-[13px] text-emerald-100/80 font-medium leading-relaxed">{{ selectedJob()?.strengths }}</p>
                        </div>
                      }
                      @if (selectedJob()?.weaknesses) {
                        <div class="p-5 rounded-2xl bg-rose-500/5 border border-rose-500/20">
                          <p class="text-[11px] font-bold uppercase text-rose-400 mb-2 tracking-widest">Gaps & Weaknesses</p>
                          <p class="text-[13px] text-rose-100/80 font-medium leading-relaxed">{{ selectedJob()?.weaknesses }}</p>
                        </div>
                      }
                      @if (selectedJob()?.recommendations) {
                        <div class="p-5 rounded-2xl bg-amber-500/5 border border-amber-500/20">
                          <p class="text-[11px] font-bold uppercase text-amber-400 mb-2 tracking-widest">Recommendations</p>
                          <p class="text-[13px] text-amber-100/80 font-medium leading-relaxed whitespace-pre-line">{{ selectedJob()?.recommendations }}</p>
                        </div>
                      }
                    </div>
                    ...
                    <div class="flex flex-col gap-4 px-4 max-w-md mx-auto">
                      <div class="relative group">
                          <select [(ngModel)]="selectedResumeId" 
                                  class="w-full bg-[#111827] border border-white/10 rounded-xl py-3.5 pl-5 pr-12 text-[14px] font-medium hover:border-white/20 focus:border-indigo-400 transition-all outline-none text-white/90">
                            <option [ngValue]="null" disabled selected>Select a resume to compare...</option>
                            @for (resume of userResumes(); track resume.resumeId) {
                              <option [ngValue]="resume.resumeId" class="text-white">{{ resume.title }}</option>
                            }
                          </select>
...
export class JobsPageComponent implements OnInit {
  ...
  private mapper = inject(ResumeDataMapperService);
  ...
  analyzeJob(job: JobMatch) {
    ...
    // 1. Get full resume content to send for ATS audit
    this.resumeService.getResumeById(this.selectedResumeId).subscribe({
      next: (resume) => {
        const resumeContent = this.mapper.mapToMarkdown(resume);
        const jobDesc = job.jobDescription || 'No job description provided.';

        console.log('Resume content for analysis:', resumeContent);
        console.log('Job description for analysis:', jobDesc);

        // 2. Call the specialized ATS audit endpoint
        this.aiService.checkAts(this.selectedResumeId!, resumeContent, jobDesc).subscribe({
          next: (report: AtsReport) => {
            // 3. Map report back to job match for display
            const updated: JobMatch = {
              ...job,
              matchScore: report.score,
              verdict: report.verdict,
              strengths: report.keywordsFound.join(', '),
              weaknesses: report.keywordsMissing.join(', '),
              recommendations: report.suggestions.map(s => `[${s.priority}] ${s.category}: ${s.action}`).join('\n')
            };
