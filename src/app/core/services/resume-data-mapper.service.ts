...
export class ResumeDataMapperService {
  ...
  mapToMarkdown(resume: BackendResume): string {
    const data = this.toRenderData(resume);
    let md = `# ${data.personal.name}\n`;
    if (data.personal.subtitle) md += `**${data.personal.subtitle}**\n`;
    md += `Email: ${data.personal.email} | Location: ${data.personal.location}\n\n`;
